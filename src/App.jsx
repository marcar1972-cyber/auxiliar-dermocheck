...
import { HelpCircle, Calendar, Search, Pill, ExternalLink, AlertTriangle, X, CheckCircle2, AlertOctagon, Info, Package, Disc, Cylinder } from 'lucide-react';
...
const BRAND_GROUPS = {
  ...
  BEIERSDORF: {
    ...
    placeholder: "Ej: 51236376",
    helpText: "Busca un número largo. 1er dígito = Año. 2do y 3ro = Semana."
  }
};
...
// --- 2. LÓGICA DE NEGOCIO (ESTRICTA SEGÚN PDF) ---

// Helper: Conversión Estricta Semana -> Mes
const getMonthFromWeek = (weekInt) => {
  if (weekInt >= 1 && weekInt <= 5) return 0;   // Ene
  if (weekInt >= 6 && weekInt <= 9) return 1;   // Feb
  if (weekInt >= 10 && weekInt <= 13) return 2; // Mar
  if (weekInt >= 14 && weekInt <= 18) return 3; // Abr
  if (weekInt >= 19 && weekInt <= 22) return 4; // May
  if (weekInt >= 23 && weekInt <= 27) return 5; // Jun
  if (weekInt >= 28 && weekInt <= 31) return 6; // Jul
  if (weekInt >= 32 && weekInt <= 35) return 7; // Ago
  if (weekInt >= 36 && weekInt <= 40) return 8; // Sep
  if (weekInt >= 41 && weekInt <= 44) return 9; // Oct
  if (weekInt >= 45 && weekInt <= 48) return 10; // Nov
  if (weekInt >= 49 && weekInt <= 53) return 11; // Dic
  return -1; // Error
};

const calculateExpiration = (brandGroup, code) => {
  const cleanCode = code.toUpperCase().trim();

  // --- A. LÓGICA L'ORÉAL (Vencimiento Directo) ---
  if (brandGroup.id === 'loreal') {
    if (cleanCode.length < 4) return { error: "Código muy corto (mínimo 4 caracteres)." };

    const yearChar = cleanCode.charAt(2);  // 3er carácter
    const monthChar = cleanCode.charAt(3); // 4to carácter

    // Mapa de Años (VENCIMIENTO DIRECTO)
    ...
    return {
      ...
      mfgText: "N/A - Cifrado directo de vencimiento", 
      isSolarWarning: false
    };
  }

  // --- B. LÓGICA BEIERSDORF (Elaboración Calculada + 36 Meses) ---
  if (brandGroup.id === 'beiersdorf') {
    ...
    if (isNaN(weekDigits) || weekDigits < 1 || weekDigits > 53) {
      return { error: `La semana ${weekDigits} no es válida (Máx 53).` };
    }

    // 1. Calcular Año Elaboración
    const currentYear = new Date().getFullYear();
    const decade = Math.floor(currentYear / 10) * 10; 
    let mfgYear = decade + yearDigit;
    // Ajuste década pasada (si estamos en 2025 y sale 9, es 2019)
    if (mfgYear > currentYear + 1) mfgYear -= 10; 

    // 2. Calcular Mes Elaboración (Tabla Estricta)
    const mfgMonthIndex = getMonthFromWeek(weekDigits);

    // 3. CÁLCULO FINAL: Sumar EXACTAMENTE 3 AÑOS (36 MESES)
    const expYear = mfgYear + 3;
    ...
    return {
      ...
      expYear: expYear,
      expMonthIndex: mfgMonthIndex, // +36 meses exactos cae en el mismo mes
      isSolarWarning: true
    };
  }
...
export default function App() {
  ...
  // Identificar Grupo Activo
  const activeGroup = Object.values(BRAND_GROUPS).find(group => 
    group.brands.includes(selectedBrand)
  );
  ...
  const handleCalculate = () => {
    ...
    if (!batchCode) {
      setError("Ingresa el código del lote.");
      return;
    }

    const calc = calculateExpiration(activeGroup, batchCode);
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans flex flex-col selection:bg-emerald-500 selection:text-white">
        ...
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-500 rounded-lg flex items-center justify-center shadow-lg shadow-emerald-500/20">
              <Pill className="text-slate-900" size={24} strokeWidth={2.5} />
            </div>
            <div>
      ...
      <main className="flex-grow max-w-md mx-auto w-full p-4 space-y-6 mt-2 pb-40">
        ...
        <div className="space-y-2">
          <label className="text-emerald-400 text-xs font-bold uppercase tracking-wider ml-1">
              ...
              {Object.values(BRAND_GROUPS).map((group) => (
                <optgroup key={group.id} label={group.name} className="bg-slate-800 text-slate-300">
                  {group.brands.map(brand => (
                    <option key={brand} value={brand} className="text-white font-medium py-1">
                      {brand}
                    </option>
                  ))}
                </optgroup>
        ...
        {activeGroup && (
          <div className="space-y-2 animate-in slide-in-from-bottom-2 fade-in duration-300">
            <div className="flex justify-between items-end ml-1">
              ...
              {/* BOTÓN DE AYUDA RECUPERADO */}
              <button 
                onClick={() => setShowHelp(true)}
                className="text-xs text-emerald-300 hover:text-white flex items-center gap-1 transition-colors bg-emerald-900/30 px-2 py-1 rounded border border-emerald-500/30 hover:bg-emerald-500 hover:border-emerald-500 hover:text-slate-900"
              >
                <HelpCircle size={14} />
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-bold text-lg py-4 rounded-xl shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
        ...
        {result && (
          <div className="bg-white rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in duration-300 ring-4 ring-emerald-500/20">
            ...
            <div className="p-6 text-center bg-white relative space-y-4">
              ...
              <div className="flex flex-col items-center justify-center border-b border-slate-100 pb-4">
                <span className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold mb-1 flex items-center gap-1">
                  <Info size={10} /> Fecha Elaboración / Fabricación
                </span>
                <span className="text-slate-600 font-bold text-lg">
                  {result.type === 'CALCULATED' 
                    ? `${MONTH_NAMES[result.mfgMonthIndex]} ${result.mfgYear}`
                    : result.mfgText}
                </span>
              </div>

              {/* LÍNEA 2: FECHA DE VENCIMIENTO (Grande) */}
              <div>
                <span className="text-[10px] uppercase tracking-wider text-red-500 font-bold mb-1 block">
                  Fecha Límite de Uso
                </span>
            ...
            {/* ADVERTENCIA PARA SOLARES (SOLO BEIERSDORF) */}
            {result.isSolarWarning && (
              <div className="bg-amber-100 border-t border-amber-200 p-4">
                <div className="flex gap-3 items-start">
                  <AlertTriangle className="text-amber-600 shrink-0 mt-0.5" size={20} />
                  <div>
                    <p className="text-amber-900 font-bold text-sm">
                      ⚠️ Si es Protector Solar
                    </p>
                    <p className="text-amber-800 text-xs mt-1 leading-snug">
                      Considerar <strong>6 meses menos</strong> (Vencen a los 30 meses). La fecha de arriba es para cremas estándar (36 meses).
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-50 border-t border-slate-100 p-2 text-center">
              <p className="text-[10px] text-slate-400 uppercase tracking-widest">
                Verificar siempre el envase físico
              </p>
            </div>
          </div>
        )}
        ...
        <div className="max-w-md mx-auto py-4 px-6 text-center">
          ...
          <div className="flex flex-col items-center gap-1 mb-3">
            <p className="text-xs text-slate-400 font-semibold">
              © 2025 <span className="text-emerald-500">AuxiliarPro</span>
            </p>
            <p className="text-xs text-slate-500">
              Herramienta desarrollada con IA y puesta a disposición de forma gratuita para la comunidad de
            </p>
            <a 
              href="https://www.auxiliaresdefarmacia.cl" 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-emerald-500 text-sm font-bold hover:text-emerald-400 hover:underline flex items-center gap-1 transition-all mt-1"
            >
              www.auxiliaresdefarmacia.cl
          ...
          <div className="border-t border-slate-800 pt-2">
            <p className="text-[10px] text-slate-600 leading-tight">
              Los cálculos son estimaciones basadas en el lote. Siempre verifica la fecha impresa oficial.
            </p>
          </div>
        </div>
      </footer>

      {/* MODAL DE AYUDA (CONTENIDO ESTRUCTURADO) */}
      <Modal isOpen={showHelp} onClose={() => setShowHelp(false)} title="Ubicación del Lote">
        <div className="space-y-5">
          
          {/* SECCIÓN 1: ZONAS COMUNES */}
          <div>
            <h4 className="text-emerald-600 text-xs font-bold uppercase tracking-widest mb-3 border-b border-slate-200 pb-1">
              Zonas Comunes
            </h4>
            <ul className="space-y-3">
              <li className="flex gap-3 items-start">
                ...
                <div>
                  <strong className="text-slate-800 text-sm block">Cajas</strong>
                  <span className="text-xs text-slate-500">Busca el troquelado (relieve) en la base inferior o tapas.</span>
                </div>
              </li>
              <li className="flex gap-3 items-start">
                <div className="bg-slate-100 p-2 rounded text-slate-600">
                ...
                <div>
                  <strong className="text-slate-800 text-sm block">Tubos</strong>
                  <span className="text-xs text-slate-500">En el borde superior plano (el cierre o "crimp").</span>
                </div>
              </li>
              <li className="flex gap-3 items-start">
                <div className="bg-slate-100 p-2 rounded text-slate-600">
                ...
                <div>
                  <strong className="text-slate-800 text-sm block">Botellas</strong>
                  <span className="text-xs text-slate-500">Impreso con tinta negra/azul en el fondo o dorso inferior.</span>
                </div>
              </li>
            </ul>
          </div>

          {/* SECCIÓN 2: TIP DE LECTURA */}
          {activeGroup && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3">
              <h4 className="text-emerald-800 text-xs font-bold uppercase tracking-widest mb-1">
                Tip de Lectura
              </h4>
              <p className="text-emerald-700 text-xs leading-relaxed">
                {activeGroup.helpText}
              </p>
            </div>
          )}
        </div>
